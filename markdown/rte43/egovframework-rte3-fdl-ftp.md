---
id: "egovframework-rte3-fdl-ftp"
title: "FTP"
version: "4.3"
category: "rte43"
source: "eGovFrame Wiki"
source_url: "https://www.egovframe.go.kr/wiki/doku.php?id=egovframework%3Arte3%3Afdl%3Aftp"
format: "markdown"
---

# FTP 서비스
## 개요
**전자정부 표준프레임워크**는 FTP 서비스제공을 위해 Apache Commons Net™ [단순 클라이언트측의 기본적인 Internet Protocol의 구현의 FTP기능을 편리하게
제공]을 오픈 소스로 채택하였다.
...
위배가 되는 사항이 있다는 것을 참고적으로 알고 있어야 한다.
#### FTP정의
...
##### 파일 다운로드 (get)
```java
File get_file = new File("c: temp test.jpg");
...
##### 파일 업로드 (put)
```java
File put_file = new File("c: temp test.jpg");
...
##### 파일 업로드 (append)
```java
File append_file = new File("c: temp test.jpg");
...
```
##### 파일 및 전송상태 설정
- 파일 타입 :  FTP.BINARY_FILE_TYPE,, FTP.ASCII_FILE_TYPE, 등 설정
- 파일 전송 형태 :  FTP.STREAM_TRANSFER_MODE, COMPRESSED_TRANSFER_MODE 등 설정
