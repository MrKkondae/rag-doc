---
id: "egovframework-dev-tst-run-testsuite"
title: "Run TestSuite"
version: "4.3"
category: "development"
source: "eGovFrame Wiki"
source_url: "https://www.egovframe.go.kr/wiki/doku.php?id=egovframework%3Adev%3Atst%3Arun_testsuite"
format: "markdown"
---
...
✔ 여기에서는 CI 서버에 등록하고 확인하는 방법에 대해서는 설명하지 않도록 한다. ## 환경설정
...
- pom.xml 작성 : #Test Automation을 위한 pom.xml 설정 샘플 : test와 같이 작성한다.
- m2eclipse로 테스트 수행 : Run As > Maven test 선택하여 실행한 뒤, Console View에서 **BUILD SUCCESSFUL**을 확인한다. [이미지] - XML 리포트 확인 : **<reportFormat>xml</reportFormat>** 항목에 xml, txt를 명시할 수 있으며, **TEST-{TestcaseFullName}.xml** 식의 파일이 생성된다. [이미지] ### Ant를 사용한 테스트 자동화
Ant를 사용하여 개인빌드를 하는 경우, 다음과 같이 테스트를 자동화할 수 있다.
- build.xml 작성 : #Test Automation을 위한 build.xml 설정 샘플 : junit과 같이 작성한다.
- 이클립스에서 테스트 수행
- 이클립스에서 Ant를 이용하여 target을 수행한다.
- Console View에서 실행 결과를 확인한다. [이미지] - XML 리포트 확인 : ''junit'' task에서 **''<formatter type="xml"/>''** 항목에 xml, txt, brief를 설정할 수 있으며, 위의 예에서는 build/junit-reports 디렉토리 밑에 TEST-{TestcaseFullName}.xml 식의 파일이 생성된다. [이미지] ## 샘플
