---
id: "egovframework-rte"
title: "실행환경 가이드"
version: "4.3"
category: "runtime"
source: "eGovFrame Wiki"
source_url: "https://www.egovframe.go.kr/wiki/doku.php?id=egovframework%3Arte"
format: "markdown"
---
...
[이미지]
## 목표
전자정부 개발프레임워크 실행환경은 ‘전자정부 서비스의 품질향상 및 정보화 투자 효율성 향상’을 위해 개발프레임워크 실행환경 표준을 정립하고, 개발프레임워크 표준 적용을 통한 응용 SW의 표준화 및 품질과 재사용성 향상을 목표로 한다.
## 배경
...
## 특징
개발프레임워크 실행환경은 전자정부 프레임워크에 대한 요구사항에 기반하고 기존 전자정부 프로젝트에 적용된 업체별 프레임워크를 비교 분석하여 34개 실행환경 프레임워크 서비스를 정의하였다. 개발프레임워크 실행환경은 서비스 별 오픈 소스 소프트웨어 심사 과정을 거쳐 선정하였다. 경량화된 개발프레임워크로서 사실상 업계 표준에 가까운 Spring 프레임워크를 기반으로 한다. 연계 솔루션에 독립된 전자정부 개발프레임워크 표준 연계 인터페이스를 정의하고 웹서비스 기반 구현체를 제공한다.
- 5개 서비스 그룹, 34개 서비스로 구성 - 개발프레임워크 실행환경은 화면처리, 업무처리, 데이터처리, 연계통합, 공통기반의 5개 서비스 그룹으로 구성되며, MVC, IoC 컨테이너, AOP, Data Access, Integration 등의 핵심 서비스를 제공한다.
- 전자정부 프로젝트에 최적화된 오픈 소스 소프트웨어 선정 – 개발프레임워크 실행환경은 전자정부 프레임워크에 대한 요구사항에 기반하여 34개 서비스 별 오픈 소스 소프트웨어 심사 과정을 거쳐 최적의 오픈 소스 소프트웨어를 선정하였다.
- 경량화된 개발프레임워크로서 사실상 업계 표준에 가까운 Spring 프레임워크를 적용 – J2EE 표준을 준수하는 Spring 프레임워크를 채택함으로써 특정 업체의 WAS나 DBMS에 독립적인 환경을 제공한다.
- DI(Dependency Injection) 방식의 의존 관계 처리 – 개발프레임워크 실행환경은 Dependency Injection을 통해 개체나 컴포넌트 간의 의존성을 정의함으로써 변경용이성과 재사용성을 향상한다.
- AOP(Aspect Oriented Programming) 지원 – 개발프레임워크 실행환경은 로깅, 트랜잭션, 예외처리와 같은 공통 관심 대상을 분리하여 정의함으로써, 개발자가 비즈니스 업무 개발에 집중할 수 있도록 한다.
- MVC Model2 아키텍처 구조 제공 및 다양한 UI 클라이언트 연계 지원 – 개발프레임워크 실행환경은 Spring MVC를 기반으로 하며, 다양한 UI 클라이언트 연계를 위한 인터페이스를 정의한다.
- 전자정부 개발프레임워크 표준 연계 인터페이스 정의 – 개발프레임워크 실행환경 표준 연계 인터페이스를 정의함으로써 연계 솔루션에 대한 의존성을 배제하고 독립적인 어플리케이션 개발이 가능하다.
## 실행환경 서비스 구성
전자정부 개발프레임워크 실행환경은 5개 서비스 그룹으로 구성되며 34개 서비스를 제공한다. 실행환경 서비스 그룹 및 서비스는 아래 그림과 같다.
...
| --- | --- | --- | --- | --- |
| 화면처리 | Ajax Support | [ Ajax Tags](http://ajaxtags.sourceforge.net/ ) | 1.5 |  |
|  | Internationalization | [ Spring MVC](http://www.springsource.org/ ) | 2.5 |  |
|  | MVC | [ Spring](http://www.springsource.org/ ) | 2.5 | Custom Tag 외 기능 확장 |
|  | Security | [ Apache Commons Validator](http://commons.apache.org/validator/ ) | 1.3 |  |
|  | UI Adaptor | 선정되지 않음 |  | UI Adaptor 연동 매뉴얼 제공 |
| UX처리 | UX/UI Controller Component | [ JqueryMobile](http://jquerymobile.com ) | BETA 3 |  |
...
| 업무처리 | Process Control | [ Web Flow](http://www.springsource.org/webflow ) | 2.0 |  |
|  | Exception Handling | [ Spring](http://www.springsource.org/ ) | 2.5 | Exception 기능 확장 |
| 데이터처리 | Data Access | [ iBatis SQL Maps](http://ibatis.apache.org/ ) | 2.3 | Spring-iBatis 기능 확장 |
|  | DataSource | [ Spring](http://www.springsource.org/ ) | 2.5 |  |
|  | ORM | [ Hibernate](http://www.hibernate.org/ ) | 3.2 |  |
|  | Transaction | [ Spring](http://www.springsource.org/ ) | 2.5 |  |
| 연계통합 | Naming Service Support | [ Spring](http://www.springsource.org/ ) | 2.5 |  |
|  | Integration Service | 선정되지 않음 |  | 표준 인터페이스 처리 기능 개발 |
|  | Web Service Interface | [ CXF](http://cxf.apache.org/ ) | 2.2 | 표준 인터페이스를 준수하도록 웹서비스를 확장 |
| 공통기반 | AOP | [ Spring](http://www.springsource.org/ ) | 2.5 |  |
|  | Cache | [ EHCache](http://ehcache.sourceforge.net/ ) | 1.6 |  |
|  | Compress/Decompress | [ Apache Commons Compress](http://commons.apache.org/compress/ ) | 1.0 |  |
|  | Encryption/Decryption | [ java simplified encryption (jasypt)](http://www.jasypt.org/ ) | 1.5 | 암호화 기능 확장 |
|  | Excel | [ Apache POI](http://poi.apache.org/ ), [ jXLS](http://jxls.sourceforge.net/ ) | 3.2, 0.9.8 | Excel 기능 확장 |
|  | File Handling | [ Jakarta Commons VFS](http://commons.apache.org/vfs/ ) | 1.0 | File Access 기능 확장 |
|  | File Upload/Download | [ Apache Commons FileUpload](http://commons.apache.org/fileupload/ ) | 1.2 |  |
|  | FTP | [ Apache Commons Net ](http://commons.apache.org/net/ ) | 2.0 |  |
|  | ID Generation | 선정되지 않음 |  | 시스템 고유 ID 생성 기능 개발 |
|  | IoC Container | [ Spring](http://www.springsource.org/ ) | 2.5 |  |
|  | Logging | [ Log4j](http://logging.apache.org/ ) | 1.3 |  |
|  | Mail | [ Apache Common Email](http://commons.apache.org/email/ ) | 1.1 |  |
|  | Marshalling/Unmarshalling | [ Castor](http://www.castor.org/ ), [ Apache XML Beans](http://xmlbeans.apache.org/ ) | 1.2, 2.4 |  |
|  | Object Pooling | [ Apache Commons Pool](http://commons.apache.org/pool/ ) | 1.4 |  |
|  | Property | [ Spring](http://www.springsource.org/ ) | 2.5 | Property 기능 확장 |
|  | Resource | [ Spring](http://www.springsource.org/ ) | 2.5 |  |
|  | Scheduling | [ Quartz](http://www.opensymphony.com/quartz/ ) | 1.6 |  |
|  | Server Security | [ Spring Security](http://static.springsource.org/spring-security/site/index.html ) | 2.0.4 | 인증, 권한 관리 기능 확장 |
|  | String Util | [ Jakarta Regexp](http://jakarta.apache.org/regexp/ ) | 1.5 | 문자열 처리 기능 확장 |
|  | XML Manipulation | [ Apache Xerces 2](http://xerces.apache.org/xerces2-j/ ), [ JDOM](http://www.jdom.org/ ) | 2.9, 1.1 | XML 처리 기능 확장 |
## 참고자료
- 3. 실행환경 오픈소스 소프트웨어 사용현황 오픈소스 소프트웨어 참조
