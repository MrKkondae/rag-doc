---
id: "egovframework-dev-imp-editor-dbio-editor-sql-map-editor"
title: "SQL Map Editor"
version: "4.3"
category: "development"
source: "eGovFrame Wiki"
source_url: "https://www.egovframe.go.kr/wiki/doku.php?id=egovframework%3Adev%3Aimp%3Aeditor%3Adbio_editor%3Asql_map_editor"
format: "markdown"
---
...
- Alias 설정
- 각 그룹별 ID 중복체크 기능
- Outline을 통한 각 그룹별 ID 바로가기 기능
- Custom Layout 조정 기능
## 설명
### 화면
[이미지] <sup> <그림 1> SQL Map Editor 의 화면 구성 </sup>

① SqlMap Namespace, 그룹별 구성요소 Tree 편집 화면 ② SqlMap 내에 속한  Query, ParameterMap, ResultMap, Alias 등 각 구성요소에 대한 편집 화면, 구성요소별 편집화면이 다르다. ③ 사용자 LayOut 조정 기능 (Horizontal, Vertical 선택) ④ SqlMap 구성 Outline ⑤ Query Result View 화면, Query Test 결과를 보여준다. ⑥ SqlMap 파일을 수정할 때 사용자는 Form UI 를 사용할 것인지 XML 소스를 직접 수정할 것인지 선택할 수 있다.
### 주요기능
#### SQLMap Tree
...
## 사용법
### SqlMap File 새로 만들기
- 상단 메뉴의 (("eGovFramework" 메뉴는 eGovFramework Perspective 환경에서만 나타난다.)) eGovFramework -> Implementation -> New SQL Map 또는 Context Menu의 New -> sqlMap 를 통해 파일을 생성한다. (<그림 2> 과 <그림 3> 참조)
- SqlMapConfig 파일이 위치할 폴더를 선택하고 파일명을 입력한다.(<그림 4> 참조)

[이미지] <sup> <그림 2> SQL Map 파일의 생성방법1 </sup>


[이미지] <sup> <그림 3> SQL Map 파일의 생성방법2 </sup>


[이미지] <sup> <그림 4> SQL Map 파일의 생성3 </sup>
### SqlMap Editor 열기
...
[이미지] <sup> <그림 5> SQL Map Editor 로 열리지 않는 경우  </sup>
### 신규 Query Map의 작성
...
[이미지] <sup> <그림 6> SQL Map Tree의 Context menu </sup>

[이미지] <sup> <그림 7> QueryMap 의 In/Out 설정</sup>

[이미지] <sup> <그림 8> SQL Map Editor의 기본 Query 편집화면  </sup>
### Query Builder 사용하기
...
#### Query Builder
[이미지] <sup> <그림 9> SQL Map Editor의 Query Builder 실행화면  </sup>
##### 사용할 테이블 추가하기
...
[이미지] <sup> <그림 10> SQL Map Editor의 Query Test 화면 </sup>


[이미지] <sup> <그림 11> SQL Map Editor의 Query Test 결과 화면  </sup>
...
[이미지] <sup> <그림 12> SQL Map Editor의 ParameterMap 작성화면  </sup>
### 신규 ResultMap 생성
- SqlMap Editor 화면 중 좌측에 있는 SQLMap Tree 에서 마우스 오른쪽 키를 누르면 context menu 가 나타난다.
...
[이미지] <sup> <그림 13> SQL Map Editor의 ResultMap 작성화면  </sup>
### Alias 설정하기
- SqlMap Editor 화면 중 좌측에 있는 SQLMap Tree 에서 마우스 오른쪽 키를 누르면 context menu 가 나타난다.
...
[이미지] <sup> <그림 14> SQL Map Editor의 Alias 작성화면  </sup>
...
[이미지] <sup> <그림 15> SQL Map Editor의 XML Source 편집  </sup>
